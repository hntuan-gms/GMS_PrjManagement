 ...
 * forward-only BFS, same FS/SS/FF/SF earliest-start formulas — but pure and
 * synchronous: no Jira writes, no overlay store, nothing awaited. Run immediately
 * after a drag (alongside, not instead of, the real PATCH request) so every task
 * the cascade would move updates on screen the instant the bar is dropped, rather
 * than waiting out the round trip to Jira for each affected successor. The PATCH
 * response remains the source of truth (applyTaskUpdate overwrites these values
 * once it resolves), so a local pass that gets a detail wrong — e.g. a concurrent
 * edit this client hasn't seen yet — self-corrects a moment later instead of
 * sticking.
 */
export function computeOptimisticCascade(
  ...
  while (queue.length > 0) {
    ...
    for (const { taskId, pred } of successorsOf.get(curId) ?? []) {
      ...
      const succEarliest = earliestStart(pred.type, curStart, curEnd, pred.lagDays, succ.durationDays);
      if (succEarliest > succ.startDate) {
        succ.startDate = succEarliest;
        succ.dueDate = addDays(succEarliest, succ.durationDays - 1);
